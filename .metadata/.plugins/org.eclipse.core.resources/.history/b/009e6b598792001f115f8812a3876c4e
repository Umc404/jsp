...
//				log.info(">>>> bvo insert 객체 >>>> {}", bvo);
				...
				// 첨부파일이 있는 경우
				// bvo를 구성하여 DB로 전송
				// file 을 저장하는 작업 = 파일이름만 DB imageFile column 에 저장.
				...
				// multipart/form-data 형식을 넘어온 request 객체를 다루기 쉽게 변환해주는 객체
...
//											.lastIndexOf(File.separator)+1);
							...
							// 아까 경로 만들어놓은 파일 fileDir + / + fileName
		...
		case "detail": case "modify":
			try {
				...
				if(path.equals("detail")) {
					...
					log.info(">>>> detail readCount >> {}", setReadCount);
				}
...
//				log.info(">>>> update >>>"+(isOk>0?"성공":"실패"));
				...
				for(FileItem item : itemList) {
					...
					case "content":
						...
						break;
					case "imageFile":
						// 기존 파일 => 있을수도 있고, 없을 수도 있고
						old_file = item.getString("utf-8");		// 파일명만 가져오기
						break;
					case "newFile":
						// 새로 추가된 파일. 있을 수도 있고 없을 수도 있고.
						if(item.getSize()>0) {
							if(old_file != null) {
								// 기존 파일이 있다면 기존파일 삭제 후 새로운놈으로 넣기
								// 파일 삭제작업 : 별도 핸들러로 작업 진행
							...
							try {
								...
								Thumbnails.of(uploadFilePath)		// 기준파일
								.size(75, 75)
								.toFile(new File(fileDir+File.separator+"_th_"+fileName));
							...
							// 기존파일은 있지만, 새로운 이미지 파일이 없다면..
							// 기존 객체를 담기. 안담으면 null 처리됨;
